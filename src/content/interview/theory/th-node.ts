...
<ul>
<li><b>Сетевой I/O</b> (TCP, HTTP) использует системные механизмы epoll, kqueue, IOCP — ожидание соединений не занимает по потоку на каждое. Поэтому тысячи одновременных соединений возможны, если хватает памяти, файловых дескрипторов и лимитов ОС; бесконечной пропускной способности это не даёт.</li>
